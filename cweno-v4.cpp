...
int main()
{
  ...
  weight[0].resize(imax);
  weight[1].resize(imax);
  weight[2].resize(imax);

   //initiate original weigh
  ...
  //array for storing weight in interpolation calculation
  float w[3];
  ...
  do{
    ...
    vec = sin(vec);
    if (sign_f(vec) >0) up = 0;
    if (sign_f(vec) <0) up = 1;

    up = 3; // match first index for the interpolation loop
    for(i=3; i<imax-4; ++i){
      stn[0] = u[i-3];
	 stn[1] = u[i-2];
	 stn[2] = u[i-1] ;
